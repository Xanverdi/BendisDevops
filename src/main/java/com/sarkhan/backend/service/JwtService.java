...
public class JwtService {
    ...
    public boolean tokenControl(String jwt, UserDetails userDetails) {
        String email = findEmail(jwt);
        String storedToken = (String) redisTemplate.opsForValue().get("access_token:" + email);

        return (email.equals(userDetails.getUsername()) &&
                jwt.equals(storedToken) &&
                !exportToken(jwt, Claims::getExpiration).before(new Date()));
    }

    public String generateToken(UserDetails user) {
        return generateToken(new HashMap<>(), user, 86400000); // 24 saat
    }

    public String generateRefreshToken(UserDetails user) {
        return generateToken(new HashMap<>(), user, 604800000); // 7 gün
    }
